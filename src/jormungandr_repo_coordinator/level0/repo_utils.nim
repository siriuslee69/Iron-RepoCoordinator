# ==================================================
# | Jormungandr Repo Coordinator Repo Utilities    |
# |------------------------------------------------|
...
const
  ...
  RootsEnv* = "JRC_ROOTS"
  OwnersEnv* = "JRC_OWNERS"
  ForeignModeEnv* = "JRC_FOREIGN_MODE"
  JrcConfigFile* = "valkyrie/jrc.toml"
...
proc readJrcConfig*(r: string): JrcConfig =
  ...
  let p = joinPath(r, JrcConfigFile)
  if fileExists(p):
...
proc ownerAllowed*(c: JrcConfig, owner: string): bool =
  ## c: jrc config with owner list.
  ## owner: repo owner/user string.
  if c.owners.len == 0:
    return true
  result = c.owners.contains(owner.toLowerAscii())

proc ownersConfigured*(c: JrcConfig): bool =
  ## c: jrc config to check.
  result = c.owners.len > 0

proc ownerWriteAllowed*(c: JrcConfig, owner: string): bool =
  ## c: jrc config with owner list.
  ## owner: repo owner/user string.
  if owner.len == 0:
    return false
...
proc ownerUpdateAllowed*(c: JrcConfig, owner: string): bool =
  ## c: jrc config with owner list.
  ## owner: repo owner/user string.
...
proc writeLocalModules*(p: string, ms: seq[SubmoduleInfo]) =
  ...
  var t: string = "# Local submodule overrides generated by Jormungandr-RepoCoordinator\n"
  for m in ms:
    t.add("\n[submodule \"" & m.name & "\"]\n")
    t.add("  path = " & m.path & "\n")
    t.add("  url = " & m.url & "\n")
