...
proc parseEpoch(text: string): int64 =
...
proc formatEpoch(epoch: int64): string =
  ## epoch: unix epoch value.
  if epoch <= 0:
    return "unknown"
  result = fromUnix(epoch).local().format("yyyy-MM-dd HH:mm:ss")

proc readConflictFiles(repo: string): seq[string] =
...
proc readBranch(repo: string): string =
  ## repo: repo path.
...
proc readMergeBranch(repo: string): string =
  ## repo: repo path.
...
proc readHeadEpoch(repo: string): int64 =
  ## repo: repo path.
...
proc readMergeEpoch(repo: string): int64 =
  ## repo: repo path.
...
proc collectConflictRepos(rootPath: string): seq[ConflictRepoState] =
...
proc chooseColors(s: ConflictRepoState): tuple[ours: string, theirs: string] =
...
proc buildDiffLines(oursText: string, theirsText: string, maxDiffs: int): seq[string] =
...
proc showConflictFile(repo: ConflictRepoState, filePath: string) =
...
proc resolveFile(repoPath: string, filePath: string, mode: string): bool =
