# ==================================================
# | Valkyrie Repo Coordinator Repo Utilities    |
# |------------------------------------------------|
...
const
  ...
  CoordinatorConfigFile* = "valkyrie/repo_coordinator.toml"
...
proc readCoordinatorConfig*(r: string): CoordinatorConfig =
  ...
  let p = joinPath(r, CoordinatorConfigFile)
  if fileExists(p):
...
proc writeLocalModules*(p: string, ms: seq[SubmoduleInfo]) =
  ...
  var t: string = "# Local submodule overrides generated by Valkyrie-RepoCoordinator\n"
  for m in ms:
    t.add("\n[submodule \"" & m.name & "\"]\n")
    t.add("  path = " & m.path & "\n")
    t.add("  url = " & m.url & "\n")
