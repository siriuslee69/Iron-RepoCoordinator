...
proc toSystemPath(p: string): string {.role(helper).} =
...
proc cmdQuote(p: string): string {.role(helper).} =
  ## p: local path to quote for Windows cmd usage.
  result = "\"" & toSystemPath(p).replace("\"", "\"\"") & "\""

proc samePath*(a: string, b: string): bool {.role(helper).} =
...
proc looksLikeLocalPath*(u: string): bool {.role(helper).} =
...
proc readRepoOrigin*(r: string): string {.role(parser).} =
  ## r: repo path.
...
proc readRepoBranch*(r: string): string {.role(parser).} =
  ## r: repo path.
...
proc setRepoOrigin*(r: string, u: string): int {.role(actor).} =
  ## r: repo path.
...
proc dirtyItemCount*(r: string): int {.role(helper).} =
  ## r: repo path.
  var
...
proc readCommitEpoch*(r: string): int64 {.role(parser).} =
  ## r: repo path.
  var
    t: tuple[text: string, code: int]
    s: string
...
proc localNameForSubmodule*(m: SubmoduleInfo): string {.role(helper).} =
...
proc candidateLocalNames*(m: SubmoduleInfo): seq[string] {.role(helper).} =
  ## m: submodule metadata.
  var
    names: seq[string]
    t: string
  proc addName(v: string) {.role(helper).} =
    let s = v.strip()
    if s.len == 0:
      return
...
proc preferredExistingSiblingRepo*(rootDir: string, m: SubmoduleInfo,
                                   ...
                                   lines: var seq[string]): string {.role(helper).} =
...
proc setGitmodulesUrl*(repo: string, m: SubmoduleInfo, url: string): int {.role(actor).} =
...
proc canonicalRemoteForSubmodule*(m: SubmoduleInfo, siblingPath: string, nestedPath: string): string {.role(helper).} =
  ## m: submodule metadata.
...
proc removePathTree*(p: string): int {.role(actor).} =
...
proc createDirLink*(linkPath: string, targetPath: string): int {.role(actor).} =
...
proc copyWorktreeFiles(src: string, dst: string) {.role(actor).} =
...
proc applyDeletedPaths(src: string, dst: string) {.role(actor).} =
  ## src: source repo worktree.
  ## dst: destination repo worktree.
  var
    t: tuple[text: string, code: int]
...
proc cloneRepoToPath*(source: string, dest: string): int {.role(actor).} =
...
proc copyWorktreeToRepo*(src: string, dst: string) {.role(actor).} =
...
proc choosePreferredRepo*(nestedPath: string, siblingPath: string,
                          expectedUrl: string, lines: var seq[string]): string {.role(helper).} =
...
proc linkConfiguredSubmodules*(repo: string, locals: seq[SubmoduleInfo],
                               replaceExisting: bool): SubmoduleLinkReport {.role(helper).} =
  ## repo: parent repository path.
