# ==================================================
# | iron Repo Coordinator Submodule Refresh |
# |------------------------------------------------|
...
proc runStash(r: string): int {.role(actor).} =
  ## r: repo path to stash.
  var c: string = "git -C " & quoteShell(r) & " stash push -u -m \"iron submodule stash\""
  result = execCmd(c)

proc runCheckoutMain(r: string): int {.role(actor).} =
  ## r: repo path to checkout main (create tracking if needed).
  var c: string = "git -C " & quoteShell(r) & " checkout -B main origin/main"
  result = execCmd(c)

proc runPull(r: string): int {.role(actor).} =
  ## r: repo path to pull.
...
proc refreshSubmodules*(): SubmoduleRefreshReport {.role(orchestrator).} =
  var
