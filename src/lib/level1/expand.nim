...
proc getOriginUrl(r: string): string {.role(helper).} =
...
proc matchSubmodule(m: SubmoduleInfo, n, u: string): bool {.role(helper).} =
...
proc buildLocalEntry(m: SubmoduleInfo, l: string): SubmoduleInfo {.role(truthBuilder).} =
...
proc expandSubmodule*(r: string, v: bool): ExpandReport {.role(orchestrator).} =
  ...
  if not ownersConfigured(cfg):
    ...
    addLine(report.lines, "Run `iron config` to set owners before expand.")
    result = report
    return
