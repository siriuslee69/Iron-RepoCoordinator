# ==================================================
# | iron Repo Coordinator Submodule Extract |
# |------------------------------------------------|
...
proc resolveRootDir(repo, rootOverride: string): string {.role(parser).} =
...
proc cloneRepo(url, dest: string): int {.role(actor).} =
  ## url: git remote to clone.
  ## dest: destination folder.
  var c: string = "git clone " & quoteShell(url) & " " & quoteShell(dest)
  result = execCmd(c)

proc repoOriginUrl(r: string): string {.role(helper).} =
  ## r: repo path.
...
proc repoAheadCount(r: string): int {.role(helper).} =
  ## r: repo path.
...
proc isRepoMoreRecent(r, expectedUrl: string): bool {.role(parser).} =
  ## r: repo path.
...
proc ensureLocalClone(url, dest: string, replace: bool, v: bool,
    report: var SubmoduleExtractReport): CloneOutcome {.role(actor).} =
...
proc buildLocalEntry(m: SubmoduleInfo, localPath: string): SubmoduleInfo {.role(truthBuilder).} =
...
proc upsertLocalModules(p: string, ms: seq[SubmoduleInfo]): seq[SubmoduleInfo] {.role(helper).} =
...
proc extractSubmodules*(r: string, rootOverride: string,
    replace: bool, v: bool): SubmoduleExtractReport {.role(orchestrator).} =
  ...
  if not ownersConfigured(cfg):
    report.ok = false
    addLine(report.lines, "No owners configured in " & readGlobalCoordinatorConfigPath() & ".")
    addLine(report.lines, "Run `iron config` to set owners before extract.")
    return report
...
proc extractSubmodulesGlobal*(rootOverride: string,
    replace: bool, v: bool): SubmoduleExtractGlobalReport {.role(orchestrator).} =
  ## rootOverride: optional root folder for local clones.
  ## replace: delete non-repo destinations before cloning.
  ## v: verbose output.
  var
