...
proc hasTestWord(s: string): bool {.role(parser).} =
  ## s: text to check for test keyword.
  result = s.toLowerAscii().contains("test")

proc parseTaskName(s: string): string {.role(parser).} =
...
proc parseTaskDesc(s: string): string {.role(parser).} =
  ## s: nimble task line.
  var
    iStart: int
    iStop: int
...
proc readNimbleTasks*(p, r: string): seq[TaskEntry] {.role(parser).} =
  ## p: nimble file path.
  ## r: repo root path.
  var
    tOut: seq[TaskEntry]
...
proc cleanTomlValue(s: string): string {.role(helper).} =
  ## s: toml value string.
...
proc readEitriTasks*(p, r: string): seq[TaskEntry] {.role(parser).} =
...
proc collectTestTasks*(rs: seq[string]): seq[TaskEntry] {.role(truthBuilder).} =
...
proc buildMappings*(l: int): seq[string] {.role(truthBuilder).} =
...
proc formatTaskLine*(m: string, t: TaskEntry): string {.role(actor).} =
  ## m: mapping token.
  ## t: task entry.
  var
    k: string
...
proc readChoice(ms: seq[string]): int {.role(parser).} =
  ## ms: mapping tokens.
  var
    input: string
    i: int
...
proc runProcessInDir(r: string, c: string, args: seq[string]): int {.role(actor).} =
  ## r: working directory.
...
proc splitCommand(s: string): tuple[cmd: string, args: seq[string]] {.role(parser).} =
...
proc runTask*(t: TaskEntry): int {.role(actor).} =
...
proc runTestPicker*(): int {.role(orchestrator).} =
