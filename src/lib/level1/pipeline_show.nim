# ==================================================
# | Valkyrie Tooling Pipeline Viewer               |
# |------------------------------------------------|
...
proc defaultPipelineCandidates*(repoPath: string): seq[string] =
  ...
  result = @[
    joinPath(tRepo, "valk", "pipeline.json"),
    joinPath(tRepo, "valk", "pipeline.library.json"),
    joinPath(tRepo, "valkyrie", "pipeline.json"),
    joinPath(tRepo, "valkyrie", "pipeline.library.json")
  ]
...
proc showPipeline*(repoPath: string, pipelinePath: string, once: bool, loops: int,
  ...
  while true:
    ...
    if r.ok:
      ...
      outText = "Valkyrie Pipeline Viewer\n\nError: " & r.error & "\nFile: " &
        p.replace('\\', '/')
    clearTerminal()
    echo outText
    inc i
