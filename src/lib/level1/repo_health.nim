...
proc readBranchName(r: string): string {.role(parser).} =
  ## r: repo path.
...
proc readUpstreamName(r: string): string {.role(parser).} =
  ## r: repo path.
...
proc readHasRemote(r: string): bool {.role(parser).} =
  ## r: repo path.
  var t: tuple[text: string, code: int] = runGit(r, "remote")
  result = t.code == 0 and t.text.strip().len > 0

proc readIsSubmodule(r: string): bool {.role(parser).} =
  ## r: repo path.
  var t: tuple[text: string, code: int] = runGit(r, "rev-parse --show-superproject-working-tree")
  result = t.code == 0 and t.text.strip().len > 0

proc parseStatusCountsFromLines*(ls: seq[string]): tuple[staged: int, modified: int, untracked: int] {.role(parser).} =
...
proc readStatusCounts(r: string): tuple[staged: int, modified: int, untracked: int] {.role(parser).} =
  ## r: repo path.
...
proc parseAheadBehind(t: string): tuple[ahead: int, behind: int, ok: bool] {.role(parser).} =
...
proc readAheadBehind(r: string): tuple[ahead: int, behind: int, ok: bool] {.role(parser).} =
  ## r: repo path.
...
proc buildRepoHealth*(r: string): RepoHealth {.role(truthBuilder).} =
  ## r: repo path.
  var h: RepoHealth
...
proc buildRepoHealthReport*(rs: seq[string]): RepoHealthReport {.role(truthBuilder).} =
...
proc buildHealthFlags(h: RepoHealth): seq[string] {.role(truthBuilder).} =
  ## h: repo health info.
...
proc formatRepoHealthLine*(h: RepoHealth, v: bool): string {.role(actor).} =
  ## h: repo health info.
...
proc formatRepoHealthReport*(r: RepoHealthReport, v: bool): seq[string] {.role(actor).} =
