...
proc processRepo(r: string, rs: seq[string], d: bool, tReport: var FindLocalSubmodulesReport) {.role(helper).} =
...
proc findLocalSubmodulesFromRoots*(d: bool): FindLocalSubmodulesReport {.role(orchestrator).} =
  ## d: dry-run flag.
  var
