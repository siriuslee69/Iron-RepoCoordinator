# ==================================================
# | iron Repo Coordinator Branch Mode       |
# |------------------------------------------------|
...
proc promoteNightly(r: string, report: var BranchModeReport): bool {.role(helper).} =
  ## r: repo path.
  if hasChanges(r):
    report.ok = false
    addLine(report.lines, "Repo has uncommitted changes. Aborting.")
...
proc switchBranchMode*(r: string, mode: string): BranchModeReport {.role(orchestrator).} =
  ...
  if not ownersConfigured(cfg):
    ...
    addLine(report.lines, "Run `iron config` to set owners before branch changes.")
    return report
